...
//! `nwt` quotes the worktree path and the branch in the cleanup commands it
//! prints, `swt` quotes the path of a `.swt-check` override and the command
//! lines it prints for a human, and `aws2env` quotes each credential value it
//! prints. A rule that lives in four places drifts apart, and the copy that
//! drifts is the copy nobody tested.
//!
