...
/// Run a git command in `dir`, isolated from the host's global and system
/// config, asserting success.
///
/// [`command`] sheds the whole inherited git environment, so the fixture repo
/// under `dir` is the one git operates on. Without that, a suite that runs
/// from inside this repo's own pre-commit hook lands its fixture commits in
/// the *real* repo despite `current_dir(dir)` — git exports its own location
/// into every hook, and git reads the environment ahead of the directory it
/// was pointed at.
///
/// # Panics
///
...
///
/// The single place the isolation rules are applied. [`git`] and
/// [`git_allowing_failure`] differ only in how they treat the exit status.
///
...
/// The two pins hold the host's own global and system configuration away from
/// the fixture, so a `user.email` or a `commit.gpgsign` the developer set
/// never decides what a test reads.
///
/// Pinned by `a_fixture_obeys_no_git_variable_out_of_a_hostile_environment`.
