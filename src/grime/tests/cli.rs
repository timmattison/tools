...
///
/// The example is a flag `grime` does not have, because a *missing* argument
/// stopped being one: `BRANCH` is optional, and a run without it picks the
/// default branch instead of being refused. A refusal `grime` itself owns - no
/// default branch to pick - is silenced by `-q`, and is asserted below.
///
...
fn quiet_leaves_the_usage_error_to_the_parser_that_answers_before_grime_starts() {
    ...
    let (code, stdout, stderr) = streams(&grime(elsewhere.path(), &["-q", REFUSED_FLAG, "main"]));
...
fn no_branch_measures_main_when_the_repository_holds_one() {
    ...
    assert_eq!(
        code,
        Some(CLEAN),
        "merging main into work collides with nothing\nstdout:\n{stdout}\nstderr:\n{stderr}"
    );
    assert_eq!(
        stdout, "grime: clean - merging main into HEAD hit no conflicts",
...
fn no_branch_falls_back_to_master_when_the_repository_holds_no_main() {
    ...
    assert_eq!(
        code,
        Some(CONFLICTS),
        "merging master into work rewrites the line work rewrote\nstdout:\n\
         {stdout}\nstderr:\n{stderr}"
    );
    assert_eq!(
        header(&stdout),
        "grime: conflicts - merging master into HEAD",
        "stderr:\n{stderr}"
    );
}
...
///
/// [`ERROR`] rather than a fall back to `HEAD`: a merge of HEAD into HEAD is
...
/// The refusal is `grime`'s own, so `-q` reaches it - unlike the parser's
/// refusal above, which answers before `grime` starts.
#[test]
...
/// The flag that asks for the diff of the halt after the breakdown.
///
/// A constant, because every test of the diff names it.
const DIFF_FLAG: &str = "--diff";

/// `--diff` adds the diff of the halt, and a clean merge has no halt. So a
...
#[test]
fn diff_on_a_clean_merge_prints_the_same_bytes_as_a_run_without_it() {
    let repo = independent_branches_repo();
    ...
    assert_eq!(
        code,
        Some(CLEAN),
        "--diff adds words and changes no answer, and this merge is clean\n\
         stdout:\n{stdout}\nstderr:\n{stderr}"
    );
    assert_eq!(
        String::from_utf8_lossy(&plain.stdout),
        "grime: clean - merging beta into HEAD hit no conflicts\n",
...
/// each other, so clap refuses the pair before `grime` starts, in both
/// spellings of `-q`.
///
/// A tool that obeys one of the two in silence surprises the caller who gave
/// the other. The run is on a fixture that conflicts, so a `grime` that takes
...
/// [`ERROR`], the code of every command line that `grime` cannot read.
#[test]
fn quiet_with_diff_is_a_usage_error_in_both_spellings_of_quiet() {
    let repo = conflicting_repo();

    for quiet in ["-q", "--quiet"] {
        let output = run_raw(&repo, "left", &[quiet, DIFF_FLAG, "right"]);
        let (code, stdout, stderr) = streams(&output);
...
/// A `--diff` run of the one halt of [`conflicting_repo`], standing on `left`
/// and merging `right`, with stdout on a pipe. Each variable in `environment`
/// is set to its value, or taken away for `None`.
///
...
fn diff_through_a_pipe(repo: &TestRepo, environment: &[(&str, Option<&str>)]) -> Output {
    ...
    let mut command = grime_command(repo.path(), &[DIFF_FLAG, "right"]);
    ...
    command.output().expect("failed to run grime")
}

/// A wrapper such as `viddy(1)` gives `grime` a pipe and states `COLUMNS`, and
...
fn clicolor_zero_refuses_the_color_of_a_wrapper() {
    ...
    assert!(
        stdout.contains("diff --cc shared.txt"),
        "the control: the run printed the diff of the halt:\n{stdout}"
    );
    assert!(
...
/// A pipe with no `COLUMNS` goes to a file or to another program, and `grime`
...
/// `CLICOLOR_FORCE=1 grime --diff main | less -R`.
///
/// The run states no width, so the rule of a wrapper does not fire, and the
/// color comes from `colored` alone. A `grime` that gives the answer of
