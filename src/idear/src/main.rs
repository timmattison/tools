...
#[command(
    long_about = "Recursively searches for directories that contain exactly one entry: a .idea directory. This is useful for finding JetBrains IDE project directories that may have been orphaned when you delete a project directory before closing the IDE."
)]
struct Cli {
...
fn main() -> Result<()> {
    ...
    if cli.delete || cli.dry_run {
        ...
        println!(
            "Found {} directories containing only .idea:",
            found_dirs.len()
        );
