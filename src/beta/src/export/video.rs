use super::terminal_renderer::{TerminalState, TerminalTheme};
...
fn load_font_from_path(font_path: &str) -> Result<FontVec> {
    ...
    FontVec::try_from_vec(font_data).map_err(|_| anyhow::anyhow!("Failed to parse font"))
}
...
fn generate_and_encode_video(
    ...
    println!(
        "Loaded {} fonts for better Unicode coverage",
    ...
    for frame_num in 0..total_frames {
        ...
        while event_index < recording.events.len()
            && recording.events[event_index].time <= current_time
        {
...
fn spawn_ffmpeg(
    ...
    if is_gif {
        ...
        if optimize_web {
            ...
            cmd.arg("-x265-params").arg("lossless=1");
        }
