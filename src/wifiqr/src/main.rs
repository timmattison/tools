...
struct Cli {
    ...
    #[arg(
        ...
        long,
        default_value = "1024",
        help = "Resolution of the QR code image (width and height in pixels)"
    )]
    resolution: u32,
    ...
    #[arg(
        short,
        long,
        help = "Path to an image file to use as a logo in the center of the QR code"
    )]
    logo: Option<String>,
    ...
    #[arg(
        long,
        default_value = "10.0",
        help = "Size of the logo as a percentage of the QR code (1-100)"
    )]
    logo_size: f64,
}
...
fn add_logo_to_qr(
    ...
    logo_path: &str,
    logo_size_percent: f64,
) -> Result<DynamicImage> {
    // Load the logo image
    let logo = image::open(logo_path).context("Failed to open logo image")?;
    ...
    println!(
        "Logo dimensions: original={:?}, max_size={}, desired_size={}",
