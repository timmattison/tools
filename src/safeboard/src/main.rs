...
#[command(
    long_about = "Detects invisible Unicode characters and unsafe code points that could be used in attacks"
)]
struct Cli {
    ...
    fn contains_dangerous_characters(content: &str) -> Vec<DangerousChar> {
        ...
        for (index, ch) in content.char_indices() {
            let danger = match ch {
                ...
                '\u{E000}'..='\u{F8FF}' => {
                    Some(DangerousChar::new(ch, index, "Private use area character"))
                }
...
fn main() -> Result<()> {
    ...
    println!(
        "Starting safeboard with options: audible={}, modify={}",
