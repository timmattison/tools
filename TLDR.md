...
| `workit` | Builds a Cargo workspace manifest from a directory tree — finds every `Cargo.toml` that is a package rather than a workspace, writes those directories into the members list, and refuses duplicate package names. The manifest lands beside the tree that was scanned unless `--output` names a file, and every member is written relative to the directory that holds it. A member the walk did not find is named on stderr before the rewrite drops it. |
