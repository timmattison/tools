...
| `gsw` | Git Status Watch — compact status dashboard that watches on a TTY (refresh clock in the separator) and renders once when piped; a merge or rebase in progress gets its own row, `⚠ merge` / `⚠ rebase 1/2 · 1 conflict to resolve`. Press `p` to push the current branch after a confirmation; the result says how long ago it happened and fades off the screen after a minute. Press `G` to open the issue the branch names, through a command of your own that `GSW_ISSUE_COMMAND` names (default `ggs`); where you have no such command the key is silent. On a remote shell the first `G` asks instead of running — it posts `remote shell — press G again to run <command>` and the second press runs it, because a browser opened here would open where nobody sits. While the push runs, a window under the frame shows the newest six lines from git and from any `pre-push` hook, live, so a slow hook shows its progress instead of a frozen screen. |
...
| `workit` | Builds a Cargo workspace manifest from a directory tree — finds every `Cargo.toml` that is a package rather than a workspace, writes those directories into the members list, and refuses duplicate package names, naming each one and every path that holds it in a fixed order so two runs over one tree read the same. The manifest lands beside the tree that was scanned unless `--output` names a file, and every member is written relative to the directory that holds it. A member the walk did not find is named on stderr before the rewrite drops it. An entry it cannot read is named and skipped rather than ending the walk, with the number skipped stated at the end, and only a scan that skipped something and found nothing fails. |
