...
| `gsw` | Git Status Watch — compact status dashboard that watches on a TTY (refresh clock in the separator) and renders once when piped; a merge or rebase in progress gets its own row, `⚠ merge` / `⚠ rebase 1/2 · 1 conflict to resolve`. Press `p` to push the current branch after a confirmation; the result says how long ago it happened and fades off the screen after a minute. Press `G` to open the issue the branch names, through a command of your own that `GSW_ISSUE_COMMAND` names (default `ggs`); where you have no such command the key is silent. While the push runs, a window under the frame shows the newest six lines from git and from any `pre-push` hook, live, so a slow hook shows its progress instead of a frozen screen. |
