...
- gsw (git status watch)
  ...
  - `G` opens the issue of the branch. A worktree branch usually names one — `issue-475` — and
    ...
    - **Where the command does not exist, `G` does nothing and says nothing** — the way an
      ...
      needs a restart.
    - **On a remote shell, `G` asks once more.** The browser opens on the machine that runs gsw, and
      on a remote shell nobody sits at that machine — so the first `G` runs nothing and puts
      `remote shell — press G again to run ggs` under the frame, where the command named is whatever
      `GSW_ISSUE_COMMAND` holds. The second `G` runs it, for as long as that message is on the
      screen. gsw decides once, at start, whether the shell is remote: it is remote when
      `SSH_CONNECTION`, `SSH_CLIENT` or `SSH_TTY` is in the environment, or when a `mosh-server`
      stands above the process — mosh carries none of those variables into the shell it starts, so
      the process tree is the only place that says so. The message fades off the screen after a
      minute. Any other key takes the offer of the second press away, so a `G` a minute later asks
      again, and so does a `G` after any other key. The offer stands exactly as long as the message
      does, so a `G` pressed while a push owns the row offers nothing: that message waits for the
      row the way any other does, and the offer starts when it gets there. The limit is a
      multiplexer that starts its server as a daemon: that server reparents to PID 1, and the walk
      up the process tree stops there. gsw answers that for Zellij through the Zellij client, which
      keeps the chain. A session viewed through tmux reports whatever stands above the tmux server.
...
- wn (what's next)
  ...
  - A plan that puts an issue before its own blocker is refused, and the run exits `2`: `the order
    ...
    neither order puts an issue before its own blocker, and once an issue adds a wait, the answer
    must be a graph and a graph cannot hold that cycle, so the run exits `2` and names both: `the
    order returns to #1 and #2, and #5 waits for #6`.
  ...
  - Three things are refused, and each of them exits `2`. A `version` that is not `1` stops the
    ...
    is not a number, a `repo` not written as `owner/name` — names the path in the document, so
    `streams[1].order[0].issue` says where to look. A cycle names the numbers that hold the knot,
    as it does for a picture and for a `Waits for` column. A `waitsFor` that names a step of its
    own stream is refused by none of those: it is an edge `order` already carries.
  ...
  - These things refuse, and each exits `2`. A directory that is in no repository `gh` can name is
    ...
    waits is refused as well, and the message names the cap of 31536000 seconds, which is one year:
    ...
    the seconds and `WN_PLAN_TIMEOUT`, and it quotes the end of what the run printed — a reader who
    paid for ten minutes must be able to tell a run that was working from a run that never started,
    and every run opens with the same event, so only the end of the stream says which of those two
    this run was. A `claude` with no account earns a message naming `claude login`. A run that
    fails for a reason only `claude` knows is named back with that reason, out of the envelope it
    printed: a failing run prints one as well, `claude` writes the sentence a reader can act on
